...
let 启用出口代理 = false, 出口代理配置 = null, parsed出口代理Address = {};
let 缓存反代IP, 缓存反代解析数组, 缓存反代数组索引 = 0, 启用反代兜底 = true, 调试日志打印 = false;
let SOCKS5白名单 = ['*tapecontent.net', '*cloudatacdn.com', '*loadshare.org', '*cdn-centaurus.com', 'scholar.google.com'];
const Pages静态页面 = 'https://sprnt123.github.io/EDT-Pages.github.io';
...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
			...
			await 反代参数获取(url);
			// 读取配置以获取出口代理设置
			await 读取config_JSON(env, host, userID, UA);
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
	...
	// 验证出口代理白名单
	const 验证出口代理白名单 = (addr) => {
		if (!出口代理配置?.白名单 || 出口代理配置.白名单.length === 0) return true; // 无白名单则全部走出口代理
		return 出口代理配置.白名单.some(p => new RegExp(`^${p.replace(/\*/g, '.*')}$`, 'i').test(addr));
	};
	const 需要走出口代理 = 启用出口代理 && 验证出口代理白名单(host);
	...
	if (启用SOCKS5反代 && (启用SOCKS5全局反代 || 验证SOCKS5白名单(host))) {
		...
		try {
			// 如果需要走出口代理，先通过 ProxyIP 连接到 SOCKS5 代理服务器，再建立隧道到目标
			if (需要走出口代理) {
				log(`[出口代理] 启用出口代理，先连接到 SOCKS5: ${parsed出口代理Address.hostname}:${parsed出口代理Address.port}`);
				// 通过 ProxyIP 连接到出口代理服务器
				const 所有反代数组 = await 解析地址端口(反代IP, parsed出口代理Address.hostname, yourUUID);
				let initialSocket = await connectDirect(parsed出口代理Address.hostname, parsed出口代理Address.port, null, 所有反代数组, 启用反代兜底);

				log(`[出口代理] 建立隧道到目标: ${host}:${portNum}`);
				try {
					// 在 ProxyIP 连接上建立 SOCKS5/HTTP 隧道到目标
					initialSocket = await 出口代理ConnectOverSocket(initialSocket, host, portNum, rawData);
				} catch (err) {
					log(`[出口代理] 隧道建立失败: ${err.message}`);
					try { initialSocket.close() } catch (e) { }
					throw new Error(`出口代理隧道建立失败: ${err.message}`);
				}
...
// 在已有连接上建立 SOCKS5/HTTP 隧道（出口代理功能）
async function 出口代理ConnectOverSocket(existingSocket, targetHost, targetPort, initialData) {
	const { username, password, hostname, port } = parsed出口代理Address;
	const 代理类型 = 出口代理配置?.类型 || 'socks5';
	const encoder = new TextEncoder();
	const decoder = new TextDecoder();

	log(`[出口代理] 在已有连接上建立 ${代理类型} 隧道 -> ${targetHost}:${targetPort}`);

	if (代理类型 === 'socks5') {
		// SOCKS5 隧道
		const writer = existingSocket.writable.getWriter();
		const reader = existingSocket.readable.getReader();
		try {
			// 发送认证方法选择
			...
			if (response.done || response.value.byteLength < 2) {
				throw new Error('[出口代理] SOCKS5 方法选择失败');
			}

			const selectedMethod = new Uint8Array(response.value)[1];
			if (selectedMethod === 0x02) {
				// 需要认证
				if (!username || !password) throw new Error('[出口代理] SOCKS5 需要认证');
				...
				if (response.done || new Uint8Array(response.value)[1] !== 0x00) {
					throw new Error('[出口代理] SOCKS5 认证失败');
				}
			} else if (selectedMethod !== 0x00) {
				throw new Error(`[出口代理] SOCKS5 不支持的认证方法: ${selectedMethod}`);
			}
			...
			response = await reader.read();
			if (response.done || new Uint8Array(response.value)[1] !== 0x00) {
				throw new Error('[出口代理] SOCKS5 连接失败');
			}
		...
		try {
			...
			while (headerEndIndex === -1 && bytesRead < 8192) {
				const { done, value } = await reader.read();
				if (done || !value) throw new Error('[出口代理] HTTP代理未返回响应');
			...
			if (headerEndIndex === -1) throw new Error('[出口代理] HTTP代理响应无效');
			const statusMatch = decoder.decode(responseBuffer.slice(0, headerEndIndex)).split('\r\n')[0].match(/HTTP\/\d\.\d\s+(\d+)/);
			const statusCode = statusMatch ? parseInt(statusMatch[1], 10) : NaN;
			if (!Number.isFinite(statusCode) || statusCode < 200 || statusCode >= 300) {
				throw new Error(`[出口代理] HTTP代理连接失败: ${statusCode}`);
			}
			...
			if (有效数据长度(initialData) > 0) {
				const dataWriter = existingSocket.writable.getWriter();
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	// 处理出口代理配置
	if (!config_JSON.出口代理) config_JSON.出口代理 = { 启用: false, 类型: 'socks5', 地址: '', 白名单: [] };
	if (config_JSON.出口代理.启用 && config_JSON.出口代理.地址) {
		try {
			parsed出口代理Address = 获取SOCKS5账号(config_JSON.出口代理.地址, config_JSON.出口代理.类型 === 'https' ? 443 : 1080);
			启用出口代理 = true;
			出口代理配置 = config_JSON.出口代理;
			log(`[出口代理] 已启用: ${config_JSON.出口代理.类型} -> ${parsed出口代理Address.hostname}:${parsed出口代理Address.port}`);
		} catch (err) {
			console.error(`解析出口代理地址失败: ${err.message}`);
			启用出口代理 = false;
		}
	} else {
		启用出口代理 = false;
